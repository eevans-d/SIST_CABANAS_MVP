...
def test_admin_accommodations():
    ...
    try:
        ...
        if response.status_code == 401:
            print_test("Admin Reservations (sin auth)", "PASS", "Requiere autenticación correctamente")
            return []
...
def test_nlu_analyze():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/nlu/analyze", 
...
def test_prereservation_flow():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/reservations/pre-reserve", 
                               json=payload, timeout=10)
        
        if response.status_code == 200 or response.status_code == 201:
            data = response.json()
            if "code" in data and "expires_at" in data:
                print_test("Pre-reserva E2E", "PASS", 
            ...
            print_test("Pre-reserva E2E", "WARN", "Fechas no disponibles (esperado con datos de prueba)")
            return None
...
def main():
    ...
    print(f"{Colors.BLUE}💡 Revisar los resultados arriba para identificar posibles problemas{Colors.END}")

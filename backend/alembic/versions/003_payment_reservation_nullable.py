...
def upgrade() -> None:
    ...
    with op.batch_alter_table('payments') as batch_op:
        batch_op.alter_column('reservation_id', existing_type=sa.Integer(), existing_nullable=False, nullable=True)


def downgrade() -> None:
    # Revertir a NOT NULL (si datos huérfanos existen fallará, decisión consciente)
    with op.batch_alter_table('payments') as batch_op:
        batch_op.alter_column('reservation_id', existing_type=sa.Integer(), existing_nullable=True, nullable=False)

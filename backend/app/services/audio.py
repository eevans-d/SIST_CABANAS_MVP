...
async def transcribe_audio(file: UploadFile) -> Dict[str, Any]:
    ...
    model = get_model()
    if model is None:
        # Entorno sin modelo instalado -> forzar low confidence
        return {"error": "audio_unclear", "confidence": 0.0}

    try:
        # faster-whisper acepta path; para evitar escribir archivo generamos un temp en memoria
        ...
        with tempfile.NamedTemporaryFile(suffix=".ogg", delete=False) as tmp:
            tmp.write(raw)
            tmp_path = tmp.name
        segments, info = model.transcribe(tmp_path, language="es")  # type: ignore
        os.unlink(tmp_path)

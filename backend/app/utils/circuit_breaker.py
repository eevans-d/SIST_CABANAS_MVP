...
class CircuitBreaker:
    ...
    def _update_state_metric(self):
        ...
        state_values = {CircuitState.CLOSED: 0, CircuitState.OPEN: 1, CircuitState.HALF_OPEN: 2}
        CIRCUIT_BREAKER_STATE.labels(circuit_name=self.name).set(state_values[self._state.state])
    ...
    async def _transition_to(self, new_state: CircuitState):
        ...
        CIRCUIT_BREAKER_STATE_CHANGES.labels(
            circuit_name=self.name, from_state=old_state.value, to_state=new_state.value
        ).inc()

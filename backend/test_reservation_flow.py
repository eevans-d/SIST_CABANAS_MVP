...
def test_nlu_availability():
    ...
    try:
        payload = {
            "text": "¿Hay disponibilidad para el 15 de noviembre para 2 personas?"
        }
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            if code:
                ...
                print_test("Pre-reserva Creada", "WARN", f"Código vacío, posible error (ID: {accommodation_id})")
                return data
        else:
            error_msg = response.text
            print_test("Pre-reserva Creada", "FAIL", f"Status: {response.status_code}, Error: {error_msg}")
            return None
...
def test_reservation_conflict():
    ...
    try:
        ...
        print_test("Conflicto", "INFO", f"Intentando reservar mismo alojamiento ID={last_used_accommodation_id} para {check_in} a {check_out}")
        ...
        if response.status_code in [200, 201]:
            ...
            if data.get("error") == "processing_or_unavailable":
                print_test("Detección Conflicto", "PASS", "Conflicto detectado como 'processing_or_unavailable'")
                return True
            elif data.get("code"):
                print_test("Detección Conflicto", "FAIL", f"No se detectó el conflicto - Se creó: {data.get('code')}")
                return False
            else:
            ...
            print_test("Detección Conflicto", "WARN", f"Status: {response.status_code}, ID: {last_used_accommodation_id}")
            return False
...
def test_nlu_pricing():
    ...
    try:
        payload = {
            "text": "¿Cuánto cuesta la cabaña familiar por 3 noches?"
        }
        response = requests.post(f"{BASE_URL}/nlu/analyze", json=payload, timeout=10)
...
def test_nlu_services():
    ...
    try:
        payload = {
            "text": "¿Qué servicios incluye la cabaña? ¿Tiene WiFi y parrilla?"
        }
        response = requests.post(f"{BASE_URL}/nlu/analyze", json=payload, timeout=10)

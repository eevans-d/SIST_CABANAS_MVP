...
async def test_whatsapp_invalid_signature(test_client, monkeypatch):  # type: ignore
    # Firma inválida -> 403
    body = {"entry": [{"changes": [{"value": {"messages": [{"type": "text", "id": "m1", "from": "54911", "timestamp": "1700000000", "text": {"body": "hola"}}], "contacts": [{"wa_id": "54911"}]}}]}]}
...
async def test_whatsapp_valid_signature_text_message(test_client, monkeypatch):  # type: ignore
    ...
    body = {"entry": [{"changes": [{"value": {"messages": [{"type": "text", "id": "m2", "from": "54922", "timestamp": "1700000100", "text": {"body": "disponible el finde?"}}], "contacts": [{"wa_id": "54922"}]}}]}]}
    raw = json.dumps(body).encode()
